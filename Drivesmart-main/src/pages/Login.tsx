...
export function Login() {
    ...
    if (isSupabaseConfigured) {
      try {
        const redirectUrl = Capacitor.isNativePlatform()
          ? `com.drivepro.app://login?role=${role}`
  ...
  const handleGoogleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (googleEmailInput) {
      if (googleEmailInput.toLowerCase() === 'udaybhanu33345@gmail.com') {
        setShowGoogleModal(false);
        await login('udaybhanu33345@gmail.com');
        navigate('/admin');
        return;
      }
                ...
                <>
                  ...
                  <div className="space-y-1 mb-8 divide-y divide-slate-100 max-h-[280px] overflow-y-auto pr-1">
                    {[
                      ...
                      {
                        name: 'Uday Bhanu',
                        email: 'udaybhanu33345@gmail.com',
                        label: 'Admin',
                      ...
                      <button
                        key={account.email}
                        onClick={async () => {
                          if (account.email.toLowerCase() === 'udaybhanu33345@gmail.com') {
                            setShowGoogleModal(false);
                            await login('udaybhanu33345@gmail.com');
                            navigate('/admin');
                            return;
